...
A teacher–student communication platform: assessment submissions, feedback comments, document sharing, weekly timetables, and live group video/audio calls with screen share.

## What's included

| Requirement | How it's implemented |
|---|---|
| 1. Report submission + feedback | Students create an **Assessment**, attach a document; teacher reviews, comments, attaches feedback docs, sets status/grade |
| 2. Timetable | Teachers add weekly time slots per course; students/teachers see a combined weekly view |
| 3. Login + signup, both roles | JWT auth, role selection (`teacher` / `student`) at signup |
...
npm run seed
```
This creates one teacher, two students, an enrolled course, two timetable slots, and one submitted assessment with a starter comment — all accounts use password `password123`:
...
Open two browser windows (or one normal + one incognito) — sign up as a teacher in one, a student in the other (or just log in with the seeded accounts above), to try the full loop:

1. Teacher signs up → creates a course → copies the course ID from the dashboard card
2. Student signs up → "Join a course" → pastes the course ID
3. Teacher adds timetable slots for the course
4. Student creates an assessment submission and uploads a report document
5. Teacher opens the assessment → comments, uploads feedback doc, sets grade/status
...
- The mesh WebRTC approach doesn't scale past roughly 6-8 simultaneous video participants; fine for teacher+small group feedback sessions
